...
public final class JPushLocalBroadcastManager {
    ...
    private final HashMap<String, ArrayList<JPushLocalBroadcastManager.ReceiverRecord>> mActions = new HashMap<String, ArrayList<JPushLocalBroadcastManager.ReceiverRecord>> ();
    private final ArrayList<JPushLocalBroadcastManager.BroadcastRecord> mPendingBroadcasts = new ArrayList<JPushLocalBroadcastManager.BroadcastRecord>();
    ...
    public boolean sendBroadcast(Intent intent) {
        ...
        synchronized (this.mReceivers) {
            ...
            if (debug) {
                LogUtils.v("JPushLocalBroadcastManager", "Resolving type " + type + " scheme " + scheme + " of intent " + intent);
            }

            ArrayList entries = (ArrayList) this.mActions.get(intent.getAction());
            if (entries != null) {
                if (debug) {
                    LogUtils.v("JPushLocalBroadcastManager", "Action list: " + entries);
                }
                ...
                for (i = 0; i < entries.size(); ++i) {
                    ...
                    if (debug) {
                        LogUtils.v("JPushLocalBroadcastManager", "Matching against filter " + receiver.filter);
                    }

                    if (receiver.broadcasting) {
                        if (debug) {
                            LogUtils.v("JPushLocalBroadcastManager", "  Filter\'s target already added");
                        }
                    } else {
                        int match = receiver.filter.match(action, type, scheme, data, categories, "JPushLocalBroadcastManager");
                        if (match >= 0) {
                            if (debug) {
                                LogUtils.v("JPushLocalBroadcastManager", "  Filter matched!  match=0x" + Integer.toHexString(match));
                            }
                            ...
                            LogUtils.v("JPushLocalBroadcastManager", "  Filter did not match: " + reason);
                        }
                    }
                }
    ...
    private void executePendingBroadcasts() {
        while (true) {
            ...
            for (int var6 = 0; var6 < brs.length; ++var6) {
                JPushLocalBroadcastManager.BroadcastRecord var7 = brs[var6];

                for (int j = 0; j < var7.receivers.size(); ++j) {
                    ((JPushLocalBroadcastManager.ReceiverRecord) var7.receivers.get(j)).receiver.onReceive(this.mAppContext, var7.intent);
                }
            }
        }
    }
    ...
    private static class BroadcastRecord {
        final Intent intent;
        final ArrayList<JPushLocalBroadcastManager.ReceiverRecord> receivers;

        BroadcastRecord(Intent _intent, ArrayList<JPushLocalBroadcastManager.ReceiverRecord> _receivers) {
            this.intent = _intent;
