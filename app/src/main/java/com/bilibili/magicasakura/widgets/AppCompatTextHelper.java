...
public class AppCompatTextHelper extends AppCompatBaseHelper<TextView> {
    ...
    void loadFromAttribute(AttributeSet attrs, int defStyleAttr) {
        TypedArray array = mView.getContext().obtainStyledAttributes(attrs, online.himakeit.skylark.R.styleable.TintTextHelper , defStyleAttr, 0);

        int textColorId = array.getResourceId(online.himakeit.skylark.R.styleable.TintTextHelper_android_textColor, 0);
        if (textColorId == 0) {
            setTextAppearanceForTextColor(array.getResourceId(online.himakeit.skylark.R.styleable.TintTextHelper_android_textAppearance, 0), false);
        } else {
            setTextColor(textColorId);
        }

        if (array.hasValue(online.himakeit.skylark.R.styleable.TintTextHelper_android_textColorLink)) {
            setLinkTextColor(array.getResourceId(online.himakeit.skylark.R.styleable.TintTextHelper_android_textColorLink, 0));
        }
        array.recycle();
    }
    ...
    public void setTextAppearanceForTextColor(int resId, boolean isForced) {
        ...
        TypedArray appearance = mView.getContext().obtainStyledAttributes(resId, online.himakeit.skylark.R.styleable.TextAppearance);
        if (appearance.hasValue(online.himakeit.skylark.R.styleable.TextAppearance_android_textColor) && isTextColorForced) {
            setTextColor(appearance.getResourceId(online.himakeit.skylark.R.styleable.TextAppearance_android_textColor, 0));
        }
        appearance.recycle();
    }
