...
public class AppCompatForegroundHelper extends AppCompatBaseHelper<View> {
    ...
    void loadFromAttribute(AttributeSet attrs, int defStyleAttr) {
        TypedArray array = mView.getContext().obtainStyledAttributes(attrs, online.himakeit.skylark.R.styleable.TintViewForegroundHelper, defStyleAttr, 0);
        if (array.hasValue(online.himakeit.skylark.R.styleable.TintViewForegroundHelper_foregroundTint)) {
            mForegroundTintResId = array.getResourceId(online.himakeit.skylark.R.styleable.TintViewForegroundHelper_foregroundTint, 0);
            if (array.hasValue(online.himakeit.skylark.R.styleable.TintViewForegroundHelper_foregroundTintMode)) {
                setSupportForegroundTintMode(DrawableUtils.parseTintMode(array.getInt(online.himakeit.skylark.R.styleable.TintViewForegroundHelper_foregroundTintMode, 0), null));
            }
            setSupportForegroundTint(mForegroundTintResId);
        } else {
            Drawable drawable = mTintManager.getDrawable(mForegroundResId = array.getResourceId(online.himakeit.skylark.R.styleable.TintViewForegroundHelper_android_foreground, 0));
            if (drawable != null) {
