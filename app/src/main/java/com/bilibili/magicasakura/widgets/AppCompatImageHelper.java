...
public class AppCompatImageHelper extends AppCompatBaseHelper<ImageView> {
    ...
    void loadFromAttribute(AttributeSet attrs, int defStyleAttr) {
        TypedArray array = mView.getContext().obtainStyledAttributes(attrs, online.himakeit.skylark.R.styleable.TintImageHelper, defStyleAttr, 0);
        // first resolve srcCompat due to not extending by AppCompatImageView
        if (mView.getDrawable() == null) {
            Drawable image = mTintManager.getDrawable(mImageResId = array.getResourceId(online.himakeit.skylark.R.styleable.TintImageHelper_srcCompat, 0));
            if (image != null) {
                setImageDrawable(image);
            }
        }
        if (array.hasValue(online.himakeit.skylark.R.styleable.TintImageHelper_imageTint)) {
            mImageTintResId = array.getResourceId(online.himakeit.skylark.R.styleable.TintImageHelper_imageTint, 0);
            if (array.hasValue(online.himakeit.skylark.R.styleable.TintImageHelper_imageTintMode)) {
                setSupportImageTintMode(DrawableUtils.parseTintMode(array.getInt(online.himakeit.skylark.R.styleable.TintImageHelper_imageTintMode, 0), null));
            }
            setSupportImageTint(mImageTintResId);
        } else if (mImageResId == 0) {
            Drawable image = mTintManager.getDrawable(mImageResId = array.getResourceId(online.himakeit.skylark.R.styleable.TintImageHelper_android_src, 0));
            if (image != null) {
                setImageDrawable(image);
            }
        }
