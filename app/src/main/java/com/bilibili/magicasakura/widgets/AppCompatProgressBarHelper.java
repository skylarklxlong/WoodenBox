...
public class AppCompatProgressBarHelper extends AppCompatBaseHelper<TintProgressBar> {
    ...
    void loadFromAttribute(AttributeSet attrs, int defStyleAttr) {
        TypedArray array = mView.getContext().obtainStyledAttributes(attrs, online.himakeit.skylark.R.styleable.TintProgressBarHelper, defStyleAttr, 0);
        if (array.hasValue(online.himakeit.skylark.R.styleable.TintProgressBarHelper_progressTint)) {
            setSupportProgressTint(mProgressTintResId = array.getResourceId(online.himakeit.skylark.R.styleable.TintProgressBarHelper_progressTint, 0));
        }
        if (array.hasValue(online.himakeit.skylark.R.styleable.TintProgressBarHelper_progressIndeterminateTint)) {
            setSupportIndeterminateTint(mIndeterminateTintResId = array.getResourceId(online.himakeit.skylark.R.styleable.TintProgressBarHelper_progressIndeterminateTint, 0));
        }
        array.recycle();
    }
