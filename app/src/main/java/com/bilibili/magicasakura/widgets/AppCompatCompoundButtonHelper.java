...
public class AppCompatCompoundButtonHelper extends AppCompatBaseHelper<CompoundButton> {
    ...
    void loadFromAttribute(AttributeSet attrs, int defStyleAttr) {
        TypedArray array = mView.getContext().obtainStyledAttributes(attrs, online.himakeit.skylark.R.styleable.TintCompoundButtonHelper, defStyleAttr, 0);
        if (array.hasValue(online.himakeit.skylark.R.styleable.TintCompoundButtonHelper_compoundButtonTint)) {
            mCompoundButtonTintResId = array.getResourceId(online.himakeit.skylark.R.styleable.TintCompoundButtonHelper_compoundButtonTint, 0);
            if (array.hasValue(online.himakeit.skylark.R.styleable.TintCompoundButtonHelper_compoundButtonTintMode)) {
                setSupportButtonDrawableTintMode(DrawableUtils.parseTintMode(array.getInt(online.himakeit.skylark.R.styleable.TintCompoundButtonHelper_compoundButtonTintMode, 0), null));
            }
            setSupportButtonDrawableTint(mCompoundButtonTintResId);
        } else {
            Drawable drawable = mTintManager.getDrawable(mCompoundButtonResId = array.getResourceId(online.himakeit.skylark.R.styleable.TintCompoundButtonHelper_android_button, 0));
            if (drawable != null) {
