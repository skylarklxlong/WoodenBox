...
public class AppCompatBackgroundHelper extends AppCompatBaseHelper<View> {
    ...
    void loadFromAttribute(AttributeSet attrs, int defStyleAttr) {
        initPadding();
        TypedArray array = mView.getContext().obtainStyledAttributes(attrs, online.himakeit.skylark.R.styleable.TintViewBackgroundHelper, defStyleAttr, 0);
        if (array.hasValue(online.himakeit.skylark.R.styleable.TintViewBackgroundHelper_backgroundTint)) {
            mBackgroundTintResId = array.getResourceId(online.himakeit.skylark.R.styleable.TintViewBackgroundHelper_backgroundTint, 0);
            if (array.hasValue(online.himakeit.skylark.R.styleable.TintViewBackgroundHelper_backgroundTintMode)) {
                setSupportBackgroundTintMode(DrawableUtils.parseTintMode(array.getInt(online.himakeit.skylark.R.styleable.TintViewBackgroundHelper_backgroundTintMode, 0), null));
            }
            setSupportBackgroundTint(mBackgroundTintResId);
        } else {
            Drawable drawable = mTintManager.getDrawable(mBackgroundResId = array.getResourceId(online.himakeit.skylark.R.styleable.TintViewBackgroundHelper_android_background, 0));
            if (drawable != null) {
